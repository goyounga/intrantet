...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "돋움"; font-size: x-small;';	//Verdana -> 돋움 2009.02.20 김정균 수정
this.imgURL = '/images/editimg/';
this.debug  = 0;
...
this.btnList = {
    ...
    "italic":         ['Italic',               '이태릭',             'editor_action(this.id)',  'ed_format_italic.gif'],
    ...
    "orderedlist":    ['InsertOrderedList',    '번호삽입',       'editor_action(this.id)',  'ed_list_num.gif'],
    "unorderedlist":  ['InsertUnorderedList',  '기호삽입',      'editor_action(this.id)',  'ed_list_bullet.gif'],
    "outdent":        ['Outdent',              '좌로이동',    'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               '우로이동',    'editor_action(this.id)',  'ed_indent_more.gif'],
    "forecolor":      ['ForeColor',            '글자색상',         'editor_action(this.id)',  'ed_color_fg.gif'],
    "backcolor":      ['BackColor',            '배경색상',   'editor_action(this.id)',  'ed_color_bg.gif'],
    "horizontalrule": ['InsertHorizontalRule', '선긋기',    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           '하이퍼링크',    'editor_action(this.id)',  'ed_link.gif'],
    ...
    "htmlmode":       ['HtmlMode',             'HTML 소스보기',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          '확장하기',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
    "about":          ['about',                'Editor 정보',  'editor_about(\''+objname+'\')',  'ed_about.gif'],
...
function editor_action(button_id) {
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(_editor_url + "js/popups/editor_help.html", 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
    window.open(_editor_url + "js/popups/fullscreen.html?||"+objname+"||"+UPLOAD_IMG_PATH+"||"+UPLOAD_FOLDER_NAME+"||"+UserID,
                'FullScreen',
                'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550');
    return;
  }
  ...
  // Custom2
    ...
    var myText = showModalDialog(_editor_url + "js/popups/custom2.html",
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "js/popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "js/popups/insert_image.html?||" + UPLOAD_IMG_PATH+"||"+UPLOAD_FOLDER_NAME+"||"+UserID, editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog(_editor_url + "js/popups/insert_table.html?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
  ...
  // 파일 다운로드 후 interactive로 변경되기 때문에 주석처리한다. 2009/03/26 김정균
 ...
 // src="' +_editor_url+ '/js/popups/blank.html"
...
function editor_about(objname) {
  showModalDialog(_editor_url + "js/popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}
 ...
 * 웹에디터의 세로스크롤바 위치 셋팅
 * 주로 초기화 시킬때 사용
 * ex) editor_scrollTop("txtaCont", 0);
 */
function editor_scrollTop(objname, odnPx) {
  var editor_obj = document.all["_" +objname + "_editor"];

  var isTextarea = (editor_obj.tagName.toLowerCase() == 'textarea');

  if (isTextarea) { editor_obj.scrollTop = odnPx; }
  else            { editor_obj.contentWindow.document.body.scrollTop = odnPx; }	//실제로 여기를 탄다. textarea 아님
}
