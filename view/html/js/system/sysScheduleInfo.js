...
//	|| ((f.dept_cd.value=="04" && parent.parent.f.nex_dept_cd.value=="01"))	//고객만족본부 - 경영관리팀-전은정 : 일단 hard cording
	){
		document.getElementById("ScheduleNotice").style.display = "";
	}
	fQuery.view_org_2.value = top.f.view_org_2.value;
	chgDept(fQuery.view_org_2.value);
	//로딩 시 쿼리 자동 실행
...
function getMonth(flag)
{
	...
	sParam += day;
	sParam += "&view_org_2=" + fQuery.view_org_2.value;
	sParam += "&view_org_3=" + fQuery.view_org_3.value;
	//화면 초기화
	...
	trans.setMyUserParams("dept_cd", fQuery.view_org_2.value)
		...
		if (ht.get("cen_sch_sbjt") != "")
		{
			for (var j=i; j < aArry.length; j++)
			{
				...
				if (day == ht2.get("date"))
				{
					...
					sHTML += "<span onclick=showDetail_obj("+j+","+index+") style='font-weight:bold;font-color:red' style=cursor:hand title="+ht2.get("cen_sch_cont")+"><font id='chgtitle' onmouseover=boldChgOver(this,'OVER') onmouseout=boldChgOut(this,'OUT') style='font-weight:normal;font-color:black'>" + changeTitle + "</font></span>&nbsp;<br>"
...
function showDetail_obj(index, divindex, arg3, arg4)
{
	...
	divPlan.style.top  = 180;
	divPlan.style.left = 400;
...
function chgDept(dept_cd)
{
	...
	trans.setWait(false);
	trans.setMyUserParams("id",   "view_org_3");
	trans.setMyUserParams("etc1",   dept_cd);
...
function callbackChgDept(dept_cd)
{
	...
	fQuery.view_org_3.value = dept_cd;
	getMonth('');
}
