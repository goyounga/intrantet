...
/**
*    5분짜리 배치.. 바뀐거 ==> ICNAT03
*    ACS 발송이력 네이버에 전송하기 uc_acs_snd_hst
*/
public class ICNAT03 extends Thread  implements ILogger {
...
//			 
			...
			sql.append("     AND  rownum <= 100      "); //FOR UPDATE 중복으로 실행될때 잘못된경우 대비 5분마다 돌아서 안함.
			...
			if(cnt > 0){	//건수가 있을때만 전송
				...
				Hashtable hsRlt = HostUtil.resolvXmlResult("ICNAT03", result); //전송한 값에 대한 결과 받기
				if("00000".equals((String)hsRlt.get("result"))){ //성공
					//성공일때 성공쿼리 처리
